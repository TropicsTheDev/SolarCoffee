...
context("Inventory Page", () => {
  ...
  it("is the inventory page", () => {
    cy.get("#inventoryTitle").contains("InventoryDashboard");
  });

  it("has buttons to add inventory and receive a shipment", () => {
    cy.get("#addNewBtn > .solar-button").contains("Receive Shipment");
  });

  it("Has add new item modal on click", () => {
    cy.get("addNewBtn > .solar-button").click();
    cy.get("#modalTitle").contains("Add enw Product");
